<footer class="footer">
	<div class="containerFull">
		<div class="row">
			<div class="col-md-6">
				<div class="leftFooter">
					<p class="title textFooter text-uppercase fontWeight700">Our Address</p>
					<h4 class="text mt-lg-3 mt-2">
						407 South 10TH ST Suite 103



						Philadelphia, PA 19147<br class="d-none d-lg-block"></h4>
					<p class="title textFooter text-uppercase mt-4 fontWeight700">Drop us a line</p>
					<h4 class="text mt-lg-3 mt-2"><a href="mailto:info@andanigroup.com">info@affinitybuildersco.com</a></h4>
					<p class="title textFooter text-uppercase mt-4 fontWeight700">Call us</p>
					<h4 class="text mt-lg-3 mt-2"><a href="tel:+91 9320003577">+91 9999999999</a></h4>
				</div>
			</div>
			<div class="col-lg-4 col-md-6 offset-lg-1">
				<div class="row">
					<div class="col-md-6 col-6">
						<div class="itemFooter">
							<h4 class="small_heading fontHeading text-white">Company</h4>
							<ul>
								<li><a href="index.php">Home</a></li>
								<li><a href="about.php">About Us</a></li>
								<li><a href="services.php">Our Services</a></li>
								<li><a href="contact.php">Contact Us</a></li>
							</ul>
						</div>
					</div>
					<div class="col-md-6 col-6">
						<div class="itemFooter">
							<h4 class="small_heading fontHeading text-white">Quick Links</h4>
							<ul>
								<li><a href="blogs.php">Latest Blogs</a></li>
								<li><a href="#">Terms &amp; Condition</a></li>
								<li><a href="#">Privacy Policy</a></li>
								<li><a href="#">Sitemap</a></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="containerFull">
		<div class="copyRight d-flex flex-wrap align-items-center justify-content-between">
			<p class="textFooter">© 2024 Affinity Builders | All Rights Reserved</p>
			<div class="socialMediaIcons mt-md-3 mt-lg-0">
				<ul>
					<li><a class="facebook" href="#"><i class="fab fa-facebook-square"></i> Facebook</a></li>
					<li><a class="instagram" href="#"><i class="fab fa-instagram"></i> Instagram</a></li>
					<li><a class="twitter" href="#"><i class="fab fa-twitter"></i> Twitter</a></li>
					<li><a class="youtube" href="#"><i class="fab fa-youtube"></i> Youtube</a></li>
				</ul>
			</div>
		</div>
	</div>
</footer>